...
Analyzer::Analyzer() :
	...
	image_sub_ = it_.subscribe("/camera/image", 1, &Analyzer::imageCb, this,
			hints);
	...
	cv::createTrackbar("Canny edge min: ", OPENCV_HOUGH_UTIL, &canny_min,
			canny_max);
	cv::createTrackbar("Hough min: ", OPENCV_HOUGH_UTIL, &hough_min, hough_max);
	cv::createTrackbar("Hough line: ", OPENCV_HOUGH_UTIL, &hough_line_min,
			hough_line_max);
	cv::createTrackbar("Hough gap: ", OPENCV_HOUGH_UTIL, &hough_gap_min,
			hough_gap_max);
}
...
void Analyzer::imageCb(const sensor_msgs::ImageConstPtr& msg) {
	...
	for (size_t i = 0; i < lines.size(); i++) {
		...
		if (angle > 1 * M_PI / 6 && angle < 5 * M_PI / 6) {
			cv::line(hdst,	p1, p2, cv::Scalar(0, 0, 255), 3);
	...
	cv::line(hdst, best1, best2, cv::Scalar(255, 0, 0), 3);
	cv::resize(hdst, hdst, cv::Size(hdst.cols / 2, hdst.rows / 2));
