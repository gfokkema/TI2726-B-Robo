...
static const char LISTDESCRIPTION[] = "multimaster_msgs_fkie/ListDescription";
  ...
  class ListDescriptionResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      uint32_t length_robot_type = strlen( (const char*) this->robot_type);
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t length_robot_imagesi = strlen( (const char*) this->robot_images[i]);
      ...
      uint32_t length_robot_descr = strlen( (const char*) this->robot_descr);
